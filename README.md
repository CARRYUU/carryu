...
| ESLint                                 | Used to check if JavaScript code conforms to rules (syntax checking, reminding to delete more than code, etc.), ensuring your code quality is above a certain level.<br/><br/> 用於檢查 JavaScript 程式碼是否符合規則(語法檢查、提醒刪除多於程式碼等等)，確保你的程式碼品質在一定的水準之上 。                                                                   |
